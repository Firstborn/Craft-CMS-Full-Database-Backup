 ...
 * @since     1.0
 */
class FullDbBackupTool extends BaseTool
{
	// Public Methods
	// =========================================================================
	...
	{
        $pluginSettings = craft()->plugins->getPlugin('dbbackup')->getSettings();
        $includeTables = $pluginSettings->includeTables;
        $excludeTables = false;
        foreach($includeTables as $table => $value){
            if (!$value){
                $excludeTables[] = $table;
            }
        }

        //pass false argument to get full database backup
        if (count($excludeTables) == 0){
            $excludeTables = false;
        }

        $file = craft()->db->backup($excludeTables);
